...
class J2A:
    ...
    _Header = misc.NamedStruct(
        "4s|signature/L|magic/L|headersize/h|version/h|unknown/L|filesize/L|crc32/L|setcount"
    )
    _defaultconfig = {
        "palette": "Diamondus_2.pal",
        "compress_method": 9,
        "null_image": "error",
        "null_mask": "ignore",
        "fake_size_and_crc": None,
        "empty_set": None,
    }
    _error_action = {
    ...
    class Set:
        ...
        _Header = misc.NamedStruct(
            "4s|signature/B|animcount/B|samplecount/h|framecount/l|priorsamplecount/l|c1/l|u1/l|c2/l|u2/l|c3/l|u3/l|c4/l|u4"
        )
        _HeaderMelk = misc.NamedStruct(
            "4s|signature/B|animcount/B|samplecount/h|framecount/l|priorsamplecount/l|c1/l|u1/l|c2/l|u2/l|c3/l|u3/l|c4/l|u4/1024s|palette"
        )
        ...
        def read(f, crc, isMelk):
            ...
            assert (setheader["signature"], setheader["u1"], setheader["u2"]) == (
                b"ANIM",
                J2A.Animation._Header.size * setheader["animcount"],
                J2A.Frame._Header.size * setheader["framecount"],
        ...
        def unpack(self):
            if self._chunks:
                animinfo, frameinfo, imagedata, sampledata = (
                    zlib.decompress(c, zlib.MAX_WBITS, usize)
                    for c, usize in self._chunks
                )
                ...
                offsets = sorted(
                    (info[key], 2 * i + j)
                    for i, info in enumerate(frameinfo)
                    for j, key in enumerate(("imageoffset", "maskoffset"))
                )
                ...
                data = sorted(
                    (i1, None) if o1 == -1 else (i1, imagedata[o1:o2])
                    for (o1, i1), (o2, i2) in pairwise(offsets)
                )
                frames = (
                    J2A.Frame.read(info, img, mask)
                    for ((i1, img), (i2, mask)), info in zip(
                        grouper(data, 2), frameinfo
                    )
                )
                self._anims = [
                    J2A.Animation(
                        frames=take(info["framecount"], frames), fps=info["fps"]
                    )
                ...
                if len(self._samples) != self._samplecount:
                    print(
                        "Warning: internal sample miscount (expected: %d, got: %d)"
        ...
        def _compress(animinfo, frameinfo, imagedata, sampledata, config):
            ...
            if isinstance(compress_method, int):
                return [
                    (zlib.compress(as_compressible(c), compress_method), len(c))
                    for c in udata
                ]
        ...
        def serialize(self, config):
            ...
            return b"".join(
                [J2A.Set._Header.pack(**setheader)] + [c[0] for c in self._chunks]
            )
        ...
        def pack(self, config):
            ...
            if not self._chunks:
                ...
                animinfo = J2A.Animation._Header.iter_pack(
                    {"framecount": len(a.frames), "fps": a.fps, "reserved": 0}
                ...
                for anim in self._anims:
                    for f in anim.frames:
                        ...
                        img_data += struct.pack(
                            "<HH",
                            (width | 0x8000 if f.tagged else width),
                            (height | 0x8000 if f.truecolor else height),
                        )

                        if no_pixmap:
                            null_pixmaps += 1
                        else:  # Image and mask data should be aligned to a 4-byte boundary
                            img_data += f._rle_encoded_pixmap + b"\x00" * (
                                -len(f._rle_encoded_pixmap) & 3
                            )
                ...
                if null_pixmaps > 0:
                    J2A._error_action[config["null_image"]](
                        "found %d frames with null images" % null_pixmaps,
                        J2A.J2APackingError,
                    )
                if null_masks > 0:
                    J2A._error_action[config["null_mask"]](
                        "found %d frames with null masks" % null_masks,
                        J2A.J2APackingError,
                    )
                ...
                for frame_info in l_frameinfo:
                    ...
                    frame_info["maskoffset"] = (
                        maskoffset + img_length if maskoffset != -1 else -1
                    )
                ...
                self._chunks = J2A.Set._compress(
                    animinfo, frameinfo, img_data + mask_data, sampledata, config
                )
    ...
    class Frame:
        ...
        _Header = misc.NamedStruct(
            "H|width/H|height/h|coldspotx/h|coldspoty/h|hotspotx/h|hotspoty/h|gunspotx/h|gunspoty/l|imageoffset/l|maskoffset"
        )
        ...
        def __init__(
            ...
            if rle_encoded_pixmap is not None:
                ...
                if not truecolor:
                    self._pixmap = [
                        bytearray(row) for row in grouper(pixmap.tobytes(), width)
                    ]  # bytes
                else:
                    self._pixmap = [
                        [tuple(row[n : n + 4]) for n in range(0, len(row), 4)]
                        for row in grouper(pixmap.tobytes(), width * 4)
                    ]  # tuples
                    ...
                    for row in self._pixmap:
                        while True:
                            ...
                            while length:
                                ...
                                encoded.append(m ^ 0x80)
                                encoded += bytearray(
                                    [channel for pixel in row[:m] for channel in pixel]
                                )
                                row = row[m:]
                                length -= m
                        encoded.append(0x80)
        ...
        def autogenerate_mask(self):
            ...
            if not self.truecolor:
                for i in range(len(mask)):
                    mask[i] = sum(
                        bool(pix) << j for j, pix in enumerate(take(8, pix_iter))
                    )
            else:
                for i in range(len(mask)):
                    mask[i] = sum(
                        bool(pix[3]) << j for j, pix in enumerate(take(8, pix_iter))
                    )
            self.mask = mask
            return self
    ...
    class Sample:
        ...
        def read(raw, offset):
            ...
            loop = (
                (header["loop_start"], header["loop_end"], bool(header["flags"] & 0x10))
                if header["flags"] & 0x18
                else None
            )
            sample_data_size = header["nsamples"] * (1 + is_16bit) * (1 + is_stereo)
            sample_data_offset = (
                offset
                + J2A.Sample._Header.size
                + (0 if not header["has_appendix"] else 0x9E)
            )

            assert (header["riff_id"], header["format"], header["sc_id"]) == (
                b"RIFF",
                b"AS  ",
                b"SAMP",
            ), "signatures mismatch"
            assert header["total_size"] - header["riff_size"] == 0xC, "sizes mismatch"
            assert (header["total_size"] - header["sc_size"]) & -2 == 0x18, (
                "sizes mismatch"
            )
            ...
            sample_data = raw[
                sample_data_offset : sample_data_offset + sample_data_size
            ]
        ...
        def serialize(self):
            ...
            retval = (
                J2A.Sample._Header.pack(**header) + self._data + b"\x00" * (datalen & 1)
            )
    ...
    def read(self, isMelk):
        ...
        with open(self.filename, "rb") as j2afile:
            ...
            try:
                ...
                assert (
                    alibheader["signature"],
                    alibheader["magic"],
                    alibheader["headersize"],
                    alibheader["version"],
                ...
                if raw:
                    print(
                        "Warning: extra %d bytes found at the end of J2A file %s. Ignoring..."
                        % (len(raw), self.filename),
                        file=sys.stderr,
                    )
                    crc = zlib.crc32(raw, crc)
                if crc & 0xFFFFFFFF != alibheader["crc32"]:
                    print(
                        "Warning: CRC32 mismatch in J2A file %s. Ignoring..."
                        % self.filename,
                        file=sys.stderr,
                    )
            except (AssertionError, struct.error):
                raise J2A.J2AParsingError(
                    "File %s is not a valid J2A file" % self.filename
                )
    ...
    def render_bitdepth_appropriate_pixelmap(self, frame):
        return (
            self.render_paletted_pixelmap(frame)
            if not frame.truecolor
            else self.render_pixelmap(frame)
        )
