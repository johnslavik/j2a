...
def show_anim(set_num, anim_num):
    ...
    extremes = (
        (borders[:, 0].min(), borders[:, 1].max()),
        (borders[:, 2].min(), borders[:, 3].max()),
    )

    fig, ax = plt.subplots()
    artists = [
        [plt.imshow(image, animated=True, extent=borders[i])]
...
def generate_compmethod_stats(filename, starting_set=0):
    ...
    def dump(f, raw, setnum, chknum, level, method, wbits, memLevel, strategy):
        ...
        f.write(
            struct.pack(
                setnum, chknum, level, method, wbits, memLevel, strategy, length
            )
        )
    ...
    with open(filename, "wb") as f:
        for setnum, s in enumerate(anims.sets):
            ...
            for chknum, chk in enumerate(s._chunks):
                ...
                [
                    dump(
                        f, raw, setnum, chknum, level, method, wbits, memLevel, strategy
                    )
...
def repacking_test():
    ...
    for i, (pset, nset) in enumerate(zip(pristine_chunks, new_chunks)):
        for chunk_num, pchunk, nchunk in zip(range(4), pset, nset):
            if zlib.decompress(pchunk[0], zlib.MAX_WBITS, pchunk[1]) != zlib.decompress(
                nchunk[0], zlib.MAX_WBITS, nchunk[1]
            ):
...
def _random_pixmap(seed=None, width=260, height=80):
    ...
    a0 = b"".join(
        b * times
        for times, b in zip(
            gen(lambda: random.randrange(255), width * height),
            itertools.cycle([b"\x00", b"\xff"]),
        )
    )
...
def sample_serializing_test():
    ...
    for set_num, s in enumerate(anims.sets):
        samplesdata = bytearray(
            zlib.decompress(s._chunks[3][0], zlib.MAX_WBITS, s._chunks[3][1])
        )
        ...
        for sample_num, sample in enumerate(s.samples):
            ...
            for sprop in J2A.Sample.__slots__:
                assert getattr(re_sample, sprop) == getattr(sample, sprop), (
                    "Matching failed on set %d, frame %d" % (set_num, sample_num)
                )
            ...
            try:
                assert samplesdata.startswith(newsampledata), (
                    "Matching failed on set %d, frame %d" % (set_num, sample_num)
                )
            except:
...
def gen_bigimage(filename):
    ...
    for set_num, s in enumerate(anims.sets):
        ...
        for anim_num, anim in enumerate(s.animations):
            frame = more_itertools.first_true(
                anim.frames, None, lambda frame: frame.shape > (1, 1)
            )
